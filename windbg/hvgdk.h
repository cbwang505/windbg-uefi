...
#define STATUS_SUCCESS                   ((NTSTATUS)0x00000000)    // ntsubauth
...
//
typedef UINT32 HV_STATUS, *PHV_STATUS;
...
typedef struct HV_CALL_ATTRIBUTES _HV_INPUT_MAP_GPA_PAGES
{
    ...
    HV_GPA_PAGE_NUMBER SourceGpaPageList[1];
...
typedef struct HV_CALL_ATTRIBUTES _HV_INPUT_DATA_INTERCEPT
{
	HV_PARTITION_ID PartitionId;
	HV_VP_INDEX              VpIndex;
	UINT32                   len;
    UINT8 buf[1];
} HV_INPUT_DATA_INTERCEPT, * PHV_INPUT_DATA_INTERCEPT;
...
typedef enum _HV_REGISTER_NAME
{
    ...
    HvX64RegisterCrInterceptHighMask  = 0x000E0004,
...
typedef struct HV_CALL_ATTRIBUTES_ALIGNED(16) _HV_INPUT_SET_VP_REGISTERS
{
    ...
    HV_VP_INDEX         VpIndex;
    HV_INPUT_VTL        InputVtl;
    HV_REGISTER_ASSOC   Elements[1];
} HV_INPUT_SET_VP_REGISTERS, *PHV_INPUT_SET_VP_REGISTERS;



typedef struct HV_CALL_ATTRIBUTES_ALIGNED(8) _HV_MODIFY_VTL_PROTECTION_MASK
{
	//
	// Supplies the partition ID of the partition that this request is for.
	//
	...
	HV_GPA_PAGE_NUMBER TargetGpaBase[1];


} HV_MODIFY_VTL_PROTECTION_MASK, * PHV_MODIFY_VTL_PROTECTION_MASK;

typedef struct HV_CALL_ATTRIBUTES_ALIGNED(8) _HV_INPUT_SET_PARTITION_VTL
{
	HV_PARTITION_ID     PartitionId;	
	HV_INPUT_VTL        InputVtl;
	UINT8   flag;
    UINT8 ReservedZ[6];
} HV_INPUT_SET_PARTITION_VTL, * PHV_INPUT_SET_PARTITION_VTL;
...
typedef struct HV_CALL_ATTRIBUTES_ALIGNED(8) _HV_ENABLE_VP_VTL
{
	HV_PARTITION_ID     PartitionId;
	HV_VP_INDEX         VpIndex;
	HV_INPUT_VTL        InputVtl;
	HV_INITIAL_VP_CONTEXT   VpVtlContext;
} HV_ENABLE_VP_VTL, * PHV_ENABLE_VP_VTL;
...
#define HV_VTL_MGMT 1
#define HV_VTL_HIGH 2
#define PAGE_SHIFT 12L
#define FILENAME_MAX 256
#define HYPERCALLOUTPUTOFFSET  0x20
...
#define VSM_PFN_TO_PAGE(addr)  ((addr) * VSM_PAGE_SIZE)
...
struct DECLSPEC_ALIGN(1) vmbus_channel_offer_channel {
    struct vmbus_channel_message_header header;
...
struct  DECLSPEC_ALIGN(2) gpa_range {
	u32 byte_count;
	u32 byte_offset;
	u64 pfn_array[6];
};

struct  DECLSPEC_ALIGN(2) vmbus_channel_gpadl_header {
    struct vmbus_channel_message_header header;
    u32 child_relid;
    u32 gpadl;
    u16 range_buflen;
...
struct vmbus_channel_msginfo {
    ...
    /* So far, this is only used to handle gpadl body message #1#
    struct list_head submsglist;

    /* Synchronize the request/response if needed #1#
...
struct  DECLSPEC_ALIGN(1) vmbus_channel {
    ...
    u64 outbound_page;	/* send to parent */
    u64 inbound_page;	/* receive from parent */
...
struct vmbuspipe_descriptor
{
    struct vmbuspipe_hdr hdr;
    u8 data[1];
};
#pragma pack()
